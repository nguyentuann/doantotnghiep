...
def train_one_epoch(
    ...
    grad_clip=1.0: cắt gradient norm nếu > 1 — tránh exploding gradient.
...
def run_training(
    ...
    for epoch in range(1, max_ep + 1):
        ...
        val_loss, mae_24h, mae_48h = eval_one_epoch(model, val_loader, loss_fn, device)
    ...
    _, final_mae_24h, final_mae_48h = eval_one_epoch(model, val_loader, loss_fn, device)

    print(f"\n  Kết quả tốt nhất ({model_name.upper()}):")
    ...
    log_result(str(log_path), {k: v for k, v in result.items() if k != "history"})
